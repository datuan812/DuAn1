...
public class PanelChartLine extends javax.swing.JPanel {
    ...
    private double getMax() {
        ...
        for (ModelChartLine d : list) {
            if (Double.parseDouble(d.getValue().toString()) > max) {
                max = Double.parseDouble(d.getValue().toString());
            }
        }
        return max;
    }
    ...
    protected void paintComponent(Graphics grphcs) {
        if (list.size() > 1) {
            ...
            for (ModelChartLine data : list) {
                double location = Double.parseDouble(data.getValue().toString()) * height / max;
